...
class ComputerVisionProcessor:
    ...
    def analyze_flow_visualization(self, flow_image):
        ...
        # Convert to grayscale if needed and ensure proper data type
        if len(flow_image.shape) > 2:
            gray_image = cv2.cvtColor(flow_image.astype(np.uint8), cv2.COLOR_RGB2GRAY)
        else:
            gray_image = flow_image.astype(np.uint8)  # Ensure uint8 format for OpenCV
...
if __name__ == "__main__":
    ...
    design_params = {
        ...
        'wall_temp': 1000,
        'nose_radius': 0.1,
        'thickness': 0.01,  # Added thickness parameter
